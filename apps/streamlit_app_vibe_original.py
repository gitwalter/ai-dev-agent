...
def display_results():
    ...
    if st.button("📦 Download Project Files"):
        ...
        project_name = result.project_name if result.project_name else "vibe_original_project"
        safe_name = re.sub(r'[<>:"/\\|?*\s]', '_', project_name.strip())
        safe_name = re.sub(r'_+', '_', safe_name).strip('_')
        if not safe_name:
            safe_name = "vibe_original"
        
        timestamp = datetime.now().strftime('%Y%m%d_%H%M%S')
        safe_filename = f"vibe_original_{safe_name}_{timestamp}.zip"
