...
# Import with robust path handling and debugging
try:
    from utils.parsing.output_parsers import OutputParserFactory
except ImportError as e:
    # Debug the path issue and fix it
    import sys
    import os
    from pathlib import Path
    ...
    utils_parsing_path = project_root / "utils" / "parsing"
    print(f"DEBUG: utils/parsing path: {utils_parsing_path}")
    print(f"DEBUG: utils/parsing exists: {utils_parsing_path.exists()}")
    
    if utils_parsing_path.exists():
        print(f"DEBUG: utils/parsing contents: {list(utils_parsing_path.iterdir())}")
    
    print(f"DEBUG: Current sys.path: {sys.path[:5]}")  # First 5 entries
    
    # Ensure path is set and retry
    if str(project_root) not in sys.path:
        sys.path.insert(0, str(project_root))
    ...
    try:
        from utils.parsing.output_parsers import OutputParserFactory
        print("DEBUG: Second import attempt succeeded!")
    except ImportError as retry_error:
        print(f"DEBUG: Second import attempt failed: {retry_error}")
        raise
import re
...
class CodeGeneratorState(BaseModel):
    """State for CodeGenerator LangGraph workflow using Pydantic BaseModel."""
...
class CodeGenerator(EnhancedBaseAgent):
    """
...
"""
    ...
    async def _execute_with_langchain(self, state: AgentState) -> Dict[str, Any]:
        ...
        # Enhance prompt with masters principles
        enhanced_prompt_template = self.get_enhanced_prompt(prompt_template)
        
        # Create prompt using string formatting to avoid PromptTemplate issues
        prompt_text = enhanced_prompt_template.format(
        ...
        # Apply masters principles to generated code
        masters_enhanced_result = self._apply_masters_to_generated_code(parsed_result)
        
        self.add_log_entry("info", "Successfully created structured code generation response")
        return masters_enhanced_result
    ...
    def _apply_masters_to_generated_code(self, code_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Apply software engineering masters principles to all generated code.
        
        Args:
            code_data: Generated code data structure
            
        Returns:
            Enhanced code data with masters principles applied
        """
        try:
            enhanced_code_data = code_data.copy()
            
            # Apply masters principles to source files
            if "source_files" in enhanced_code_data:
                for filename, code_content in enhanced_code_data["source_files"].items():
                    if code_content and isinstance(code_content, str):
                        # Apply masters principles
                        masters_result = self.apply_masters_to_code(
                            code_content, 
                            context={'file_path': filename, 'file_type': 'source'}
                        )
                        
                        # Update with improved code
                        enhanced_code_data["source_files"][filename] = masters_result.get('code', code_content)
                        
                        # Log compliance information
                        compliance = masters_result.get('masters_compliance')
                        if compliance:
                            self.add_log_entry("info", f"🎯 Masters compliance for {filename}: {compliance.overall_score:.2f}")
                            
                            if compliance.violations:
                                self.add_log_entry("warning", f"🎯 {len(compliance.violations)} masters violations in {filename}")
            
            # Apply masters principles to configuration files
            if "configuration_files" in enhanced_code_data:
                for filename, config_content in enhanced_code_data["configuration_files"].items():
                    if config_content and isinstance(config_content, str):
                        # Apply simpler masters principles for config files
                        masters_result = self.apply_masters_to_code(
                            config_content,
                            context={'file_path': filename, 'file_type': 'config'}
                        )
                        
                        enhanced_code_data["configuration_files"][filename] = masters_result.get('code', config_content)
            
            self.add_log_entry("info", "🎯 Applied software engineering masters principles to all generated code")
            return enhanced_code_data
            
        except Exception as e:
            self.add_log_entry("warning", f"🎯 Error applying masters principles: {e}")
            return code_data  # Return original if enhancement fails
    ...
    def _build_langgraph_workflow(self) -> StateGraph:
        """Build LangGraph workflow for CodeGenerator."""
    ...
    async def _langgraph_execute_node(self, state: CodeGeneratorState) -> CodeGeneratorState:
...
def get_graph():
    ...
    if _default_instance is None and LANGGRAPH_AVAILABLE:
        ...
        config = AgentConfig(
            ...
            name='CodeGenerator',
            description='CodeGenerator agent',
            model_name='gemini-2.5-flash'
        )
        client = get_gemini_client(agent_name='code_generator')
        _default_instance = CodeGenerator(config, gemini_client=client)
