...
class TestGeneratorState(BaseModel):
    """State for TestGenerator LangGraph workflow using Pydantic BaseModel."""
...
CRITICAL: Respond with ONLY valid JSON - no explanations, no markdown, no additional text."""
    ...
    def _build_langgraph_workflow(self) -> StateGraph:
        """Build LangGraph workflow for TestGenerator."""
    ...
    async def _langgraph_execute_node(self, state: TestGeneratorState) -> TestGeneratorState:
...
def get_graph():
    ...
    if _default_instance is None and LANGGRAPH_AVAILABLE:
        ...
        config = AgentConfig(
            ...
            name='TestGenerator',
            description='TestGenerator agent',
            model_name='gemini-2.5-flash'
        )
        client = get_gemini_client(agent_name='test_generator')
        _default_instance = TestGenerator(config, gemini_client=client)
