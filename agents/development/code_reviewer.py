...
class CodeReviewerState(BaseModel):
    """State for CodeReviewer LangGraph workflow using Pydantic BaseModel."""
...
class CodeReviewer(EnhancedBaseAgent):
    """
...
"""
    ...
    def _build_langgraph_workflow(self) -> StateGraph:
        """Build LangGraph workflow for CodeReviewer."""
    ...
    async def _langgraph_execute_node(self, state: CodeReviewerState) -> CodeReviewerState:
...
def get_graph():
    ...
    if _default_instance is None and LANGGRAPH_AVAILABLE:
        ...
        config = AgentConfig(
            ...
            name='CodeReviewer',
            description='CodeReviewer agent',
            model_name='gemini-2.5-flash'
        )
        client = get_gemini_client(agent_name='code_reviewer')
        _default_instance = CodeReviewer(config, gemini_client=client)
