...
def main():
    ...
    parser.add_argument(
        "command",
        nargs="?",
        choices=["status", "pre-merge", "post-merge", "prepare-pull", "cleanup-pull", "pre-push", "pre-commit"],
        default="status",
    ...
    try:
        if args.command == "status":
            ...
            print("Pre-push: Cleaning up databases and temporary files...")
            # Check for database files and clean up if needed
            has_db_staged = safe_git.has_database_files_staged()
            if has_db_staged:
                print("Database files detected in staging area")
                staged_db = safe_git.get_staged_database_files()
                print(f"Staged database files: {staged_db}")
            print("Pre-push cleanup completed successfully")
            sys.exit(0)
            
        elif args.command == "pre-commit":
            print("Pre-commit: Validating changes...")
            # Basic validation before commit
            has_changes, staged, unstaged = safe_git.check_git_status()
            if not has_changes:
                print("No changes to validate")
            else:
                print(f"Validating {len(staged)} staged files")
            print("Pre-commit validation completed successfully") 
            sys.exit(0)
