...
class Issue(BaseModel):
    ...
    description: constr(min_length=10, max_length=1000) = Field(
        ...
        example="The user input is not validated before processing, which could lead to security vulnerabilities."
    )
    ...
    suggestion: Optional[str] = Field(
        ...
        example="Add input validation using Pydantic models"
    )
    impact: Optional[str] = Field(
        description="Potential impact of the issue",
        example="Could allow injection attacks"
    )
...
class Recommendation(BaseModel):
    ...
    title: constr(min_length=1, max_length=200) = Field(
        ...
        example="Implement comprehensive logging"
    )
    description: constr(min_length=10, max_length=1000) = Field(
        description="Detailed description of the recommendation",
        example="Add structured logging throughout the application for better debugging and monitoring."
    )
    ...
    benefits: List[str] = Field(
        ...
        example=["Better debugging", "Improved monitoring", "Easier troubleshooting"]
    )
    steps: Optional[List[str]] = Field(
        description="Step-by-step implementation guide",
        example=["1. Install logging library", "2. Configure loggers", "3. Add log statements"]
    )
...
class CodeReviewOutput(BaseModel):
    ...
    positive_aspects: List[str] = Field(
        ...
        example=["Well-structured functions", "Good error handling", "Clear variable names"]
    )
    ...
    summary: constr(min_length=10, max_length=500) = Field(
        ...
        example="The code demonstrates good structure and follows most best practices, but needs improvements in error handling and security validation."
    )
...
class RequirementsAnalysisOutput(BaseModel):
        ...
        example=[
            {
                ...
                "priority": "high",
                "acceptance_criteria": ["User can login with valid credentials", "User can logout successfully"]
            }
        ]
    )
    ...
    technical_constraints: List[str] = Field(
        ...
        example=["Must use Python 3.9+", "Must be compatible with existing database"]
    )
    
    assumptions: List[str] = Field(
        description="List of assumptions made during analysis",
        example=["Users have basic computer literacy", "Internet connection is available"]
    )
...
class ArchitectureDesignOutput(BaseModel):
    ...
    system_overview: constr(min_length=10, max_length=1000) = Field(
        ...
        example="A microservices-based architecture with RESTful APIs for user management and data processing."
    )
    ...
    security_considerations: List[str] = Field(
        ...
        example=["JWT authentication", "HTTPS encryption", "Input validation"]
    )
    
    scalability_considerations: List[str] = Field(
        description="List of scalability considerations and strategies",
        example=["Horizontal scaling", "Load balancing", "Database sharding"]
    )
...
class SecurityAnalysisOutput(BaseModel):
    ...
    compliance_requirements: List[str] = Field(
        ...
        example=["GDPR compliance", "OWASP Top 10", "ISO 27001"]
    )
...
class DocumentationGenerationOutput(BaseModel):
    ...
    documentation_structure: List[str] = Field(
        ...
        example=["docs/", "docs/api/", "docs/architecture/", "docs/user-guide/"]
    )
    ...
    maintenance_plan: List[str] = Field(
        ...
        example=[
            ...
            "2. Monthly documentation review and cleanup",
